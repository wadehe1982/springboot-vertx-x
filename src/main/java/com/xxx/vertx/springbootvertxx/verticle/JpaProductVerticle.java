package com.xxx.vertx.springbootvertxx.verticle;

import com.fasterxml.jackson.core.JsonProcessingException;
import com.fasterxml.jackson.databind.ObjectMapper;
import com.xxx.vertx.springbootvertxx.service.ProductService;
...
import static com.xxx.vertx.springbootvertxx.utils.Constants.*;
...
public class JpaProductVerticle extends AbstractVerticle {
    ...
    private Handler<Message<String>> allProductsHandler(ProductService productService) {
        return msg -> vertx.<String>executeBlocking(future -> {
                    try {
                        future.complete(objectMapper.writeValueAsString(productService.getAllProducts()));
    ...
    public void start() throws Exception {
        ...
        eventBus.<String>consumer(ALL_PRODUCTS_ADDRESS).handler(allProductsHandler(productService));
        eventBus.<String>consumer(GET_ONE_PRODUCT_ADDRESS).handler(getOneProductHandler(productService));
        eventBus.<JsonObject>consumer(ADD_PRODUCT_ADDRESS).handler(addProductHandler(productService));
        eventBus.<Integer>consumer(DELETE_PRODUCT_ADDRESS).handler(deleteProductHandler(productService));
        eventBus.<JsonObject>consumer(PATCH_PRODUCT_ADDRESS).handler(patchProductHandler(productService));
    }
        ...
        return message -> vertx.<JsonObject>executeBlocking(
                future -> {
                    try {
                        ...
                        future.complete(JsonObject.mapFrom(productService.patchProduct(product.mapTo(ProductService.ProductDTO.class))));
                    } catch (Exception e) {
                        log.error("Failed to serialize result");
                        future.fail(e);
                    }
        ...
        return message -> vertx.<JsonObject>executeBlocking(
                future -> {
                    try {
                        ...
                        future.complete(JsonObject.mapFrom(productService.addProduct(product.mapTo(ProductService.ProductDTO.class))));
                        log.info("got one product from blocking...");
                    } catch (Exception e) {
                        log.error("Failed to serialize result");
                        future.fail(e);
